...
class Example(wx.Frame):
    ...
    def InitUI(self):
        ...
        heading = wx.StaticText(self, label='Classification ',
                                pos=(100,50), size=(200, -1))
        heading.SetFont(font)

        

        wx.StaticText(self, label='Upload Image', pos=(25, 100))
##        wx.TextCtrl(self, -1, "", pos=(140, 100)) 

        btn = wx.Button(self, label='Upload', pos=(260, 100))

        btn.Bind(wx.EVT_BUTTON, self.OnClose)
        btn = wx.Button(self, label='Preprocess', pos=(260, 150))

        btn.Bind(wx.EVT_BUTTON, self.OnClosepre)


        btn = wx.Button(self, label='Feature Extraction', pos=(260, 200))

        btn.Bind(wx.EVT_BUTTON, self.OnClosewave)

        btn = wx.Button(self, label='Result', pos=(260, 250))
    ...
    def OnClose(self, e):
        ...
        query = cv2.imread(openFileDialog.GetPath())
    ...
    def OnClosepre(self, e):
        ...
        cv2.imshow('Gabor Filter', filtered_img)
    ...
    def OnClose1(self, e):
        ...
        # load the query image and describe it
        query = cv2.imread(self.st.GetLabelText())
...
##        fig.tight_layout()
##        plt.show()
        file = self.st.GetLabelText()
        superpixel(file)
        feature ( file )
        existingcall(file , 0, type='rect')
        o1 = ertcall(file , 0, type='rect')
        ij =0
        fi= load_images("DataSet/")
        tr= loadtrain ("DataSet/")
        sv= savefile("DataSet/")
        for trf in tr :
            ii= image_similarity_vectors_via_numpy(file ,trf )
            if ii >.99:
                x=os.path.basename(file).split('-')
...
def superpixel ( filename ):
    ...
    fig = plt.figure("Pixels Extraction")
...
def existingcall(name, debug, type=None, **options):
   ...
   # ret, markers = cv2.connectedComponents(sure_bg)
    ...
    #markers = cv2.watershed(input_image1, markers)
    #input_image1[markers == -1] = [255, 0, 0]

    #cv2.imshow('Result Image', input_image1)

    imgray = cv2.cvtColor(im, cv2.COLOR_BGR2GRAY)
    ret, thresh = cv2.threshold(imgray, 110, 255, 0)
    _, contours, _ = cv2.findContours(thresh, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)

    cv2.drawContours(im1, contours, -1, (255, 0, 0), 1)

    #cv2.imshow('Existing Image', im1 )
